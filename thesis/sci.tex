...
\begin{definition}[Size of a formula]
    ...
    $$
\end{definition}
Since $\V$ is a countable set we can assume it has a full ordering $<$. Let us
extend it by saying that for each $p \in \V$: $p < $ \enquote{$\lnot$} $<$
\enquote{$($} $<$ \enquote{$)$} $<$ \enquote{$\ra$} $<$ \enquote{$\id$}. Now,
