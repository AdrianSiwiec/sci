...
\begin{definition}[Alphabet of SCI]
    ...
    \begin{itemize}
        \item $\V = \{p, q, r, ...\}$ -- a countable infinite set of atomic formulas,
        \item $\lnot$  -- unary operator \enquote{not},
        \item $\ra, \id$ -- binary operators \enquote{implication} and \enquote{identity}.
    \end{itemize}
\end{definition}
...
Since $\V$ is a countable set we can assume it has a full ordering $<$. Let us extend it by saying that for each $p \in \V$: $p < $ \enquote{$\lnot$} $<$ \enquote{$($} $<$ \enquote{$)$} $<$ \enquote{$\ra$} $<$ \enquote{$\id$}. Now, let us define a ordering of formulas $\prec$ to be a lexicographical ordering with $<$.
...
The proof of the decidability theorem can be found in \cite{suszko_bloom} (Corollary 2.4).
