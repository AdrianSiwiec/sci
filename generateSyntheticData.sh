...
#     echo $vars $size closed Done!
#   done
# done

# Random tautologies only
rm benchmark_data/synthetic_tautologies.in
touch benchmark_data/synthetic_tautologies.in
count=100
for vars in $(seq 1 4);
do
  # 5 is minimal size for a non-trivial tautology formula (I guess)
  for size in $(seq 5 50);
  do
    if [ $size -le 6 ] && [ $vars -ge 3 ] ; then
      echo Skipping $vars $size 
      continue
    fi
    if [ $size -le 8 ] && [ $vars -ge 4 ] ; then
      echo Skipping $vars $size 
      continue
    fi
    obj/test/formula_generator.e $count $size $vars 2 >>benchmark_data/synthetic_tautologies.in
